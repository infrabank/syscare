...
> **🔒 현재 보안 상태**: `admin-bookings.html`의 클라이언트 비밀번호·브라우저 세션 인증·무인증 관리 API는 안전하지 않아 비활성화했습니다. 서버 측 인증과 작업별 권한 검증 전에는 관리자 기능을 다시 활성화하지 않습니다. GA4도 개인정보 고지와 사전 동의 구조를 마련하기 전까지 로드하지 않습니다.

> **⚠️ 2026-07 신뢰성 개편 안내**: 이 문서의 일부 기능 설명(실시간 위협 카운터, 무사고 운영일수 카운터,
> 카운트다운 타이머, 한정 특가, 고객 후기·기관 배지, 24/7 지원 문구 등)은 검증 불가능한 정보로
> 판단되어 **사이트에서 제거되었습니다**. 현재 사이트 구조와 콘텐츠 규칙은
> [CLAUDE.md](CLAUDE.md)를 기준으로 하세요. 증빙 없는 수치·후기·인증·긴급성 요소는 다시 추가하지 않습니다.
...
├── admin-bookings.html     # 관리자 기능 비활성화 안내 (서버 인증 도입 전 사용 금지)
├── css/
...
- ✅ **연락처 정보 업데이트**: 실제 연락처로 변경 (010-3861-8079, jhw@mlkit.co.kr)
